...
#   - Ejari field/terms parser (top fields + numbered clauses)
...
# =========================
# Smart PDF extraction (pdfminer → OCR)
# =========================
...
def parse_pdf_smart(pdf_bytes: bytes) -> Dict[str, Any]:
    ...
    # If text likely image-only → OCR fallback
    if len((text or "").strip()) < 120:
        ocr_txt = _ocr_pdf_to_text(pdf_bytes, lang="eng")
        if ocr_txt:
            text = ocr_txt
            ocr_used = True

    ejari_struct = parse_ejari_text(text)
    return {"text": text, "ejari": ejari_struct, "ocr_used": ocr_used}
...
def parse_ejari_text(text: str) -> Dict[str, Any]:
    ...
    if m:
        p = m.group(1).lower()
        if p == "residential":
            p = "apartment"  # map “residential” to apartment for demo logic
        prop_type = p
    ...
    out = {
        ...
        "notice_sent_date": None,           # not present; user can fill
        "bedrooms": bedrooms,
        "area": area,
...
def lookup_rera_row(
    ...
    *,
    ...
    import pandas as pd  # local import to avoid hard dependency if caller doesn't use CSV
    ...
    if "furnished" in d.columns:
        ...
        if not pref.empty:
            d = pref
    if not {"bedrooms_min", "bedrooms_max"}.issubset(d.columns):
        return None
...
def build_report_html(text: str, result: Dict[str, Any]) -> str:
    ...
    html.append(render_highlighted_html(text=result.get("raw_text_for_report", "") or "", result=result))
