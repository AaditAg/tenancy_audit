...
def parse_ejari_text(text: str) -> Dict[str, Any]:
    ...
    # Build reconstructed source_text with visual cues (📌)
    lines: List[str] = []
    if prop_type:
    ...
    if start_date and end_date:
        lines.append(f"📌 Contract Period: From {start_date} To {end_date}")
    if annual_rent is not None:
        lines.append(f"📌 Annual Rent: AED {annual_rent:,}")
    if deposit is not None:
        lines.append(f"📌 Security Deposit Amount: AED {deposit:,}")
    lines.append("---------------------------------")
