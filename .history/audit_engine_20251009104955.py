...
def _gemini_check_clause_against_articles(
    ...
    Strategy: process articles in batches; if any batch response indicates FAIL,
    stop early. We instruct the model to return a compact label.
    """
    ...
    for i in range(0, len(articles), batch_size):
        ...
        joined = "\n- " + "\n- ".join(a.strip()[:2000] for a in chunk)  # trim per item for token safety
        prompt = (
            f"{system_prompt}\n\nClause:\n{clause}\n\nReference texts:{joined}\n\nReturn exactly one of: 'FAIL - <short reason>' or 'PASS - OK'."
        )
        try:
        ...
        if head.startswith("FAIL") or "\nFAIL" in head or head == "FAIL":
            reason = txt.split("-", 1)[1].strip() if "-" in txt else "Flagged by AI layer"
            return "fail", reason
