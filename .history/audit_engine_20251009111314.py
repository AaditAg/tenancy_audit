...
def _gemini_check_clause_against_articles(
    ...
    for i in range(0, len(articles), batch_size):
        ...
        for j, a in enumerate(chunk):
            idx = start_index + i + j
            numbered.append(f"[{idx}] {a.strip()[:2000]}")
        joined = "\n" + "\n".join(numbered)
        prompt = (
            f"{system_prompt}\n\nClause:\n{clause}\n\nReference texts (indexed):{joined}\n\n"
            "Return exactly one of: 'FAIL - <short reason> - REFS: i[,j,k]' or 'PASS - OK'."
        )
        try:
        ...
        if head.startswith("FAIL") or "\nFAIL" in head or head == "FAIL":
            # Extract REFS list if present
            reason_line = txt.splitlines()[0]
            refs: List[int] = []
            m = re.search(r"REFS\s*:\s*([0-9,\s]+)", txt, re.I)
            if m:
                try:
                    refs = [int(x.strip()) for x in m.group(1).split(',') if x.strip().isdigit()]
                except Exception:
                    refs = []
            # Reason = everything between first '-' and ' - REFS' if present
            if "-" in reason_line:
                parts = reason_line.split("-", 1)[1].strip()
            else:
                parts = "Flagged by AI layer"
            return "fail", parts.split("REFS", 1)[0].strip(), refs
