...
use_text_autofill = st.sidebar.checkbox(
    ...
    value=False,  # OFF so UI reflects exactly what's in the PDF by default
)
...
with cB:
    ...
    ptype = st.selectbox(
        "Property Type",
        options,
        index=options.index(ptype_val) if ptype_val in options else 0,
    )
...
clauses_df = pd.DataFrame(
    [{"clause": c.get("num"), "text": c.get("text", "").strip()} for c in ejari.get("clauses", [])]
)
...
st.divider()
st.caption(
    "Law 26/2007 & Law 33/2008 (tenancy, notice, eviction grounds), Decree 43/2013 (rent-increase slabs). "
